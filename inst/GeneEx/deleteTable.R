...
ui <- fluidPage(
  ...
  mainPanel(
    ...
    uiOutput('undoUI'),
    actionButton('addInteraction',"Add Interaction"),
...
server <- function(input, output) {
  ...
  observeEvent(input$dtable_cell_edit, {
    rv$data[input$dtable_cell_edit$row,(input$dtable_cell_edit$col-1)] <- input$dtable_cell_edit$value
  })
  ...
  observeEvent(input$addInteraction, {
    rowNum <- (1 + nrow(rv$data))
    dataRow <- rv$data[1,]
    
    rv$data <- rbind(dataRow,rv$data)
  })
